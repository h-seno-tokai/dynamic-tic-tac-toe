# 05. UI 設計（骨格）

最終更新: 2026-04-29

本ドキュメントは画面一覧と遷移の骨格のみ定義する。詳細ワイヤーフレーム・コンポーネント分割（ユーザー単独決定）は実装フェーズで詰める。

## 1. 画面一覧

| ID | 画面名 | 主要機能 | 経路 |
|---|---|---|---|
| S01 | タイトル / メインメニュー | モード選択（ローカル / CPU / チュートリアル / 設定 / 戦績） | `/#/` |
| S02 | チュートリアル（オンボーディング） | 初回起動時に自動表示。スキップ可。 | `/#/tutorial` |
| S03 | ルール説明 | メニューから常時アクセス可能なルール解説 | `/#/rules` |
| S04 | ローカル2人対戦設定 | プレイヤー名入力・アバター選択・ルール設定 | `/#/local/setup` |
| S05 | CPU対戦設定 | 難易度（10段階）・先手後手・ルール設定 | `/#/cpu/setup` |
| S06 | ルール設定（共通） | 盤面サイズ・駒サイズ段階・各サイズ駒数 | モーダル or サブ画面 |
| S07 | 対局画面 | 盤面・駒スタック・操作（投了/待った/履歴）・思考時間表示 | `/#/play` |
| S08 | 結果画面 | 「勝負あり！」演出 / 勝敗表示 / 再戦・メニューへ | `/#/result` |
| S09 | 設定 | BGM/SFX 音量・言語・テーマ・戦績クリア | `/#/settings` |
| S10 | 戦績 | 難易度別勝敗・最終対局日 | `/#/stats` |

## 2. 画面遷移図（テキスト）

```
[初回起動] ──→ S02 チュートリアル ──→ S01 メインメニュー
[2回目以降]                       ↗
                                  │
S01 メインメニュー ─┬─ ローカル対戦 ─→ S04 設定 ─→ S07 対局 ─→ S08 結果 ─┐
                   ├─ CPU対戦       ─→ S05 設定 ─→ S07 対局 ─→ S08 結果 ─┤
                   ├─ チュートリアル ─→ S02                              │
                   ├─ ルール        ─→ S03                              │
                   ├─ 設定          ─→ S09                              │
                   └─ 戦績          ─→ S10                              │
                                                                        │
                                ┌───────────────────────────────────────┘
                                ▼
                  [再戦] → 同じモードで S07 へ
                  [メニューへ] → S01 へ
```

## 3. 主要 UX フロー

### 3.1 初回起動フロー
1. アプリロード（モデル重みをバックグラウンド取得開始）
2. ローディング画面（重み取得進捗表示）
3. S02 チュートリアル自動表示（スキップ可、後でアクセス可能なことを明示）
4. S01 メインメニュー

### 3.2 CPU対戦フロー
1. S01 → CPU対戦選択
2. S05 で難易度・先手後手・ルール選択（戻る可）
3. S07 対局
   - プレイヤー手番: マス選択 → 駒選択（手駒 or 盤面） → 配置先選択
   - CPU手番: 思考中インジケータ + 経過時間表示 + キャンセル不可（投了は可）
   - 「待った」: 直前の自分の手を取り消し（CPU手番中は無効）
4. S08 結果 → 効果音演出 → 再戦 / メニュー

### 3.3 ローカル2人対戦フロー
1. S01 → ローカル対戦選択
2. S04 でプレイヤー1/2 の名前入力 + アバター選択（既定値あり）+ ルール選択
3. S07 対局
   - 「待った」は同卓合意の前提で常時許可
4. S08 結果 → 「勝負あり！」演出 → 勝者名アバター強調 → 再戦 / メニュー

### 3.4 ルール変更フロー
- S04/S05/S09 のいずれからも S06 ルール設定にアクセス可能
- 拡張性: 盤面サイズ N、駒サイズ段階 M、各サイズの駒数 K の3パラメータ
- プリセット（3x3 大中小2個ずつ等）+ カスタムを提供
- 不正な組み合わせ（駒の総数が盤面に対して過多/過少）はバリデーション

## 4. レスポンシブ方針

- ブレイクポイント:
  - モバイル: 〜 640px
  - タブレット: 641〜1024px
  - デスクトップ: 1025px〜
- 盤面: 短辺基準で正方形を確保。タッチ領域は最低 44x44px（iOS HIG 基準）。
- メニュー: モバイルは縦スクロール、デスクトップはグリッド。
- 設定モーダル: モバイルではフルスクリーン、デスクトップではダイアログ。

## 5. アニメーション方針

- **微細**: 駒のホバー、ボタン押下、メニュー遷移（Framer Motion の spring）
- **強調**: 駒移動・覆い被せ・「勝負あり！」演出
- `prefers-reduced-motion` を尊重し、モーション軽減モードでは fade のみに簡略化

## 6. テーマ

- ダーク / ライト の2種
- CSS variables で色定義、`data-theme="dark"` 属性でルート切替
- Tailwind の `dark:` バリアントは使わず CSS 変数経由で統一（テーマ追加が将来的に楽）
- 初期値: `prefers-color-scheme`、ユーザー切替を localStorage 永続化

## 7. アクセシビリティ実装メモ

- すべての操作にキーボード経路あり
- 盤面: 矢印キーでフォーカス移動、Enter/Space で選択
- 駒・マスに `aria-label`（「中サイズの駒、プレイヤー1」「マス B-2、空」等）
- フォーカスリングを CSS で常時可視化（`:focus-visible`）
- ライブリージョンで CPU 思考完了・勝敗を読み上げ通知

## 8. 詳細未定（実装フェーズ）

- 各画面の正式ワイヤーフレーム
- カラーパレット（テーマごとの 16色 程度）
- タイポグラフィスケール
- 具体的なコンポーネント分割粒度（ユーザー単独決定）
- アニメーション数値（duration/easing 等）
- 「勝負あり！」演出の具体（テキスト・色・効果音タイミング）
