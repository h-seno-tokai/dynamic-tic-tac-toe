...
3. 4x4 CPU 対戦の **初回リクエスト時** に ONNX モデル（~3.92 MB）+ ORT-Web wasm を遅延ロード（3x3 CPU はロード不要）
...
3. S07 対局
   - 「待った」は同卓合意の前提で常時許可
4. 終局 → S07 内モーダル（S08）→「勝負あり！」+ ファンファーレ → 勝者名アバター強調 → 再戦 / メニュー

### 3.5 駒選択ジェスチャ（toggle / replace）

- 駒の選択は **toggle / replace** 動作:
  - 同じ駒を再クリック → 選択解除
  - 別の駒をクリック → 選択切替（前選択は破棄）
- 副作用として **「自分の小さい駒の上に自分の大きい駒を被せる」操作は UI から到達不可** に
  なった（同色駒を再クリックすると選択切替に解釈される）。エンジン側 (`allowSelfCover: true`) では
  依然として合法手として扱う。意図的なトレードオフ。
