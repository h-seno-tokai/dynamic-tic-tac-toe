...
### 2.0 設計目標
- ONNX サイズ < 2MB（モバイル含めた高速ロード）
- 推論時間 < 5ms / forward pass（10秒予算で MCTS 1000+ シミュレーション可能に）
- 学習が RTX 2080 で実用範囲（数日〜2週間）に収まる
...
### 2.2 ネットワーク構造（コンパクト ResNet）

```
Input (4×4×27)
   ↓
Stem: Conv3×3 (27→64) + BN + ReLU
   ↓
ResBlock × 4
  ├ Conv3×3 (64→64) + BN + ReLU
  └ Conv3×3 (64→64) + BN + skip + ReLU
   ↓
   ┌────────────────┴────────────────┐
   ↓                                 ↓
Policy Head                      Value Head
─ Conv1×1 (64→4)                 ─ Conv1×1 (64→32) + BN + ReLU
   → reshape: PlaceLogits 64-d   ─ Global Average Pool
─ Conv1×1 (64→16)                ─ Linear (32→64) + ReLU
...
**ヘッドの conv 化** は重要決定:
- PlaceHead: `Conv1×1(64→4)` → 出力 4×4×4 = 64 logits（4 sizes × 16 cells）
- MoveHead: `Conv1×1(64→16)` → 出力 4×4×16 = 256 logits（16 to-cells × 16 from-cells）
...
|---|---|
| Stem | 27 × 64 × 9 ≈ 15.6K |
| ResBlock × 4 | 4 × 2 × 64 × 64 × 9 ≈ 295K |
| Policy Heads (Place + Move) | 64 × (4 + 16) ≈ 1.3K |
| Value Head | 64 × 32 + 32 × 64 + 64 ≈ 4.2K |
| **合計** | **約 316K params** |

**ONNX 想定サイズ**: 約 1.2 MB (float32) / 約 320 KB (int8 量子化後)

### 2.4 学習ハイパーパラメータ初期値

| 項目 | 初期値 |
|---|---|
| 自己対戦並列度 | 8〜16（CPU コア数依存） |
| 学習バッチサイズ | 1024 |
...
| リプレイバッファ | 50K games（FIFO） |
| ターゲットネット更新頻度 | 1000 train steps |
| プリセットサンプル比 | 3x3:4x4 = 50:50（学習中観察し調整） |
...
### 2.6 設計決定の根拠
- **小規模 ResNet（4 ブロック・64 ch）**: 4×4 の小盤面なら過剰でない。AlphaGo Zero の 19×19 用 39 ブロック・256 ch から 1/100 規模で十分な棋力に到達できる見込み。
