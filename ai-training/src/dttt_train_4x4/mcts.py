"""Single-tree leaf-batched MCTS for the 4x4-only pipeline.

Direct port of :mod:`dttt_train.mcts`. The only behavioural delta is that
``state.to_move`` and ``state.outcome`` come from the fast engine and use
``int`` codes for side-to-move (0 = P1, 1 = P2). The :class:`Player` enum is
still used for outcomes so the legacy augmentation/encoding stay compatible;
we convert as needed inside ``_simulate_once``.

The legal-action mask is applied here in Python — it deliberately does not
appear in the network forward graph so that the ONNX export stays free of
``-inf + softmax`` patterns.
"""
...
from .engine import GameState, apply_move, is_terminal, legal_moves
...
def make_torch_evaluator(network: object) -> Evaluator:
    ...
    The network's value head is WDL (3 logits). We softmax to a (W, D, L)
