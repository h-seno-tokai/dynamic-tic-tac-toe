"""Best-network gating tournament for the 4x4-only pipeline.

Direct port of :mod:`dttt_train.eval_match`. Differences:
  * imports the fast 4x4 engine / encoding;
  * uses int side-to-move codes (the fast engine uses ``int`` for ``to_move``).

Routes leaf evaluations to whichever network is on-move at the leaf so a
candidate-vs-best tournament is symmetric under colour swap. Runs ``num_games``
games alternating colours with low-temperature, no-Dirichlet MCTS.
"""
...
def _play_one_game(
    ...
    side_eval = eval_p1 if int(state.to_move) == 0 else eval_p2
    ...
    while not is_terminal(state):
        ...
        for _ in range(num_sims):
            ...
            if is_terminal(leaf):
                oc = _outcome_code(leaf.outcome)
                if oc is None:
                    v = 0.0
                else:
                    v = 1.0 if oc == int(leaf.to_move) else -1.0
                _backup(path, v, int(leaf.to_move))
                continue
            leaf_node = path[-1]
            if leaf_node.is_expanded:
                # Already expanded by a prior selection; just back-up its current Q.
                _backup(path, _q(leaf_node), int(leaf.to_move))
                continue
            leaf_eval = eval_p1 if int(leaf.to_move) == 0 else eval_p2
            ep, ev = leaf_eval([leaf])
            _expand(leaf_node, leaf, ep[0])
            _backup(path, float(ev[0]), int(leaf.to_move))
    ...
    oc = _outcome_code(state.outcome)
    if oc is None:
        return "draw"
    return "P1" if oc == 0 else "P2"
