"""Checkpoint -> ONNX exporter with parity check, 4x4-only network.

Mirrors ``dttt_train.export_onnx`` exactly:
...
from dttt_train.rules import MAX_BOARD

from .network import INPUT_CHANNELS, DTTTNet4x4
...
def _load_checkpoint(ckpt_path: Path, prefer_ema: bool = True) -> DTTTNet4x4:
...
def export_to_onnx(net: DTTTNet4x4, out_path: Path) -> None:
    net.eval()
...
def parity_check(net: DTTTNet4x4, onnx_path: Path, num_samples: int = 4) -> float:
    net.eval()
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Export DTTT 4x4-only checkpoint to ONNX with parity check"
    )
    parser.add_argument("--ckpt", type=str, required=True, help="path to .pt checkpoint")
    parser.add_argument("--out", type=str, default="model_4x4.onnx", help="output ONNX path")
