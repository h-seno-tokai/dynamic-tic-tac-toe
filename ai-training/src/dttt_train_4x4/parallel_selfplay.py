"""Parallel self-play for the 4x4-only pipeline (fast engine port).

Identical algorithm to :mod:`dttt_train.parallel_selfplay`:
  * Tree reuse between moves   (subtree of chosen action becomes new root)
  * Playout Cap Randomization  (KataGo)
  * FPU reduction              — un-visited children inherit parent_q - fpu
  ...
  * Dirichlet root noise        re-applied per move

The only differences from the legacy port are the engine/encoding imports and
a small adapter for the new state representation (``state.to_move`` is now an
``int`` 0/1 rather than a ``Player`` enum, and ``state.outcome`` is still
``Player | "draw" | None``). We compare side-to-move and outcomes via integer
codes throughout this file.
...
from .engine import GameState, apply_move, initial_state, is_terminal
from .rules import GameRules, Player
...
# 4x4 averages roughly ~64 legal moves mid-game.
...
# ---------------------------------------------------------------------------
# Outcome / side-to-move adapters
# ---------------------------------------------------------------------------
#
# The fast engine stores ``state.to_move`` as ``0`` (P1) / ``1`` (P2). The
# outcome is still the legacy :class:`Player` enum (or "draw"/None) so the
# augmentation/encoding layer remains compatible with the legacy stack. We
# therefore convert outcomes to integer codes when comparing against
# ``to_move``.


def _outcome_code(outcome) -> int | None:
    """Return 0 if P1 won, 1 if P2 won, ``None`` for draw / unfinished."""
    if outcome is None:
        return None
    if outcome == "draw":
        return None
    if outcome is Player.P1:
        return 0
    if outcome is Player.P2:
        return 1
    return None
...
class _Node:
    ...
    to_move: int | None = None  # 0 = P1, 1 = P2
...
def _expand_node(node: _Node, state: GameState, priors: np.ndarray) -> None:
    ...
    node.to_move = int(state.to_move)
...
def _backup(path: list[_Node], leaf_value: float, leaf_player: int) -> None:
...
class _Ctx:
    ...
    # history: per stored sample (tensor, policy, mover_int, root_q)
    history: list[tuple[np.ndarray, np.ndarray, int, float]] = field(default_factory=list)
    done: bool = False
...
def play_games_parallel(
    ...
    while any(not c.done for c in ctxs):
        ...
        for sim_step in range(max_sims):
            ...
            if eval_idx:
                ...
                for j, i in enumerate(eval_idx):
                    ...
                    _backup(paths[i], float(ev[j]), int(leaf_states[i].to_move))

            for i in term_idx:
                ls = leaf_states[i]
                oc = _outcome_code(ls.outcome)
                if oc is None:
                    v = 0.0
                else:
                    v = 1.0 if oc == int(ls.to_move) else -1.0
                _backup(paths[i], v, int(ls.to_move))

            # Free virtual loss for the rare "already expanded" path: back-up
            # using the node's current Q estimate.
            for i in other_idx:
                node = paths[i][-1]
                _backup(paths[i], _q(node), int(leaf_states[i].to_move))
        ...
        for ctx in active:
            ...
            if store_sample:
                ...
                rq = _root_q(ctx.root)
                ctx.history.append((tensor, policy, int(ctx.state.to_move), rq))
    ...
    for ctx in ctxs:
        oc = _outcome_code(ctx.state.outcome)
        samples: list[Sample] = []
        for tensor, policy, mover, rq in ctx.history:
            if oc is None:
                v = 0.0
            else:
                v = 1.0 if oc == mover else -1.0
